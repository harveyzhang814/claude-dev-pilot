...
1. 启动应用并完成引导；应用会在 `~/.agent-dev-pilot/token` 生成 Bearer 令牌（权限 `0600`）。
2. 在应用内使用 **Hook 安装** 相关说明：会将 `notify.sh` / `cursor-notify.sh` 写入 `~/.agent-dev-pilot/hooks/`（内容以 `HookInstaller` 内嵌脚本为准）。
3. **Claude Code：** 按提示合并 hook 配置；脚本向 `POST /event` 上报，需携带 `Authorization: Bearer <token>`。
4. **Cursor：** 按提示将 hook 合并到 `~/.cursor/hooks.json`；`cursor-notify.sh` 向 `POST /cursor-event` 上报。可通过环境变量 `AGENT_DEV_PILOT_PORT` 覆盖端口（默认与上述端口一致）。
...
| `AgentDevPilot` | `Sources/App/` | SwiftUI 可执行文件，依赖 Core + Server |
| `AgentDevPilotTests` | `Tests/`（不含 `ServerTests`） | Core / App 单元测试 |
...
- 本地 SQLite（WAL）：`~/Library/Application Support/AgentDevPilot/db.sqlite`
- 令牌与 hook 脚本：`~/.agent-dev-pilot/`
